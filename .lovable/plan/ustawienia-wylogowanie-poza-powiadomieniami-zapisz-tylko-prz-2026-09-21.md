...
- Przycisk „Zapisz ustawienia": `disabled={saving || !dirty}`; gdy brak zmian, etykieta zostaje „Zapisz ustawienia" (bez zmian tekstu).
- Po udanym zapisie `invalidateQueries` odświeża profil → `dirty` wraca do `false`, przycisk gaśnie do kolejnej zmiany, tekst na przycisku zmienia się na 'Zapisano zmiany". Toast „Ustawienia zapisane" bez zmian.
